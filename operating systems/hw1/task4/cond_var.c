...
void condition_variable_wait(condition_variable* cv, ticket_lock* ext_lock) {
   ...
   // while (atomic_load(&cv->signals) == 0) {
     //   sched_yield(); // Yield CPU to other threads
    //}
    //atomic_fetch_sub(&cv->signals, 1);    // One thread clears the signal (for signal, not broadcast)

    atomic_fetch_sub(&cv->waiters, 1);    // Mark as no longer waiting

    int my_ticket = atomic_fetch_add(&ext_lock->ticket, 1);    // Re-acquire external lock
    while (atomic_load(&ext_lock->cur_ticket) != my_ticket) {
        sched_yield();
    }
